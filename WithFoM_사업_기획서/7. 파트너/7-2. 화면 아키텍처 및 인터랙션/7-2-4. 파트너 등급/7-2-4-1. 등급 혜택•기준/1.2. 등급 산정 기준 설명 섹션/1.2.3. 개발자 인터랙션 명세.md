...
```json
{
  "displayCriteria": {
    ...
    "scanRate": {
      ...
      "silver": { "displayMin": 80, "displayMax": 99, "label": "80~99%" },
      "gold": { "displayMin": 100, "displayMax": 114, "label": "100~114%" },
      "platinum": { "displayMin": 115, "displayMax": null, "label": "115% 초과" }
    },
    "withfomOrder": {
      "label": "위드폼 오더 이용",
      "displayScore": 10
...
```json
{
  "actualCriteria": {
    ...
    "scanRate": {
      ...
      "silver": { "actualMin": 80, "actualMax": 99 },
      "gold": { "actualMin": 100, "actualMax": 114 },
      "platinum": { "actualMin": 115, "actualMax": null }
    },
    "withfomOrder": {
      "requiredForPlatinum": true,
      "mustBeActiveEntirePeriod": true,
      "actualScore": 10
    }
  },
  "systemNotes": {
    "platinumRequirement": "위드폼 오더 미이용 시 골드까지만 달성 가능",
    "platinumOrderCondition": "산정 기간 내 위드폼 오더를 항상 이용해야 플래티넘 인정. 중간 비활성화 기간 존재 시 플래티넘 제외",
...
　B. 툴팁: role="tooltip"  
　C. 도움말 아이콘: aria-label="실 사용 기간 계산 방법 보기"  

2: 키보드 내비게이션  
　A. Tab: 섹션 내 요소 간 이동  
　B. Enter/Space: 도움말 아이콘 클릭 시 툴팁 표시  
　C. Escape: 툴팁 닫기  
...
　C. unmatchedMonths >= 6: 등급 변경 처리 및 알림  
...
　C. CTA (멤버십 상태별 분기)  
　　a. 베이직: { text: "매칭 확률 UP", action: "navigate", target: "/partner/membership/subscribe" }  
　　b. 플러스: { text: "프리미엄으로 매칭 확률 UP", action: "navigate", target: "/partner/membership/upgrade" }  
　　c. 프리미엄 + 플래티넘: { text: "매칭 시까지 등급이 유지됩니다", action: "none" } (정보성 안내)  
　　d. 프리미엄 + 골드 이하: { text: "등급 UP으로 매칭 확률 UP", action: "navigate", target: "/partner/grade/status" }  
...
5: 알림 메시지 (등급 변경 완료)  
　A. 제목: "등급 변경 안내"  
　B. 본문: "등급이 {previousGrade}에서 {newGrade}으로 변경되었습니다."  
　C. CTA (멤버십 상태별 분기)  
　　a. 베이직/플러스: { text: "매칭 확률 UP", action: "navigate", target: "/partner/membership/subscribe" } + 보조 메시지: "멤버십 업그레이드로 매칭 우선순위를 높여보세요"  
　　b. 프리미엄: { text: "등급 현황 보기", action: "navigate", target: "/partner/grade/status" }  
...
**1.2.3.7.** 등급 변경 후 재산정 로직  

1: 재산정 트리거  
　A. 등급 변경 후 새 캠페인 종료 시 자동 재산정  
