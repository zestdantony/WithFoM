...
**3.9.4.10.** 게임 분석 데이터 연동  

1: 게임 활성화 여부(7-2-7-1)에 따라 게임 분석 섹션 표시/숨김 결정  
2: 플러스(3.3.3.8.): GET /api/v1/partner/store/game-analytics/participation 호출(3.9.1.24. 참조)  
3: 프리미엄(3.3.4.10.): GET /api/v1/partner/store/game-analytics/distribution 호출(3.9.1.25. 참조)  
4: 게임 미활성화 시 "게임을 활성화하면 참여 분석을 확인할 수 있습니다" placeholder + 게임 설정(7-2-7-1) 바로가기  


**3.9.1.24.** 게임 참여율·사용률 조회(플러스)  

1: 엔드포인트  
　A. `GET /api/v1/partner/store/game-analytics/participation`  
　B. 인증: Bearer Token(파트너 세션), 플러스 이상  
...
| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| preset | string | N | today \| week \| month \| custom |
| startDate | string | N | YYYY-MM-DD |
| endDate | string | N | YYYY-MM-DD |
...
```json
{
  ...
  "freePlayUsageRate": 0.0,
  "gameDistribution": [
    { "gameName": "string", "usageRate": 0.0 }
  ],
  "previousPeriodChange": 0.0,
  "isGameActive": true
}
```
...
4: 에러  
　A. 403: 플러스 미만  
　B. SM_024: 게임 참여율 조회 실패  


**3.9.1.25.** 게임 고객 분포·재방문 효과 조회(프리미엄)  

1: 엔드포인트  
　A. `GET /api/v1/partner/store/game-analytics/distribution`  
　B. 인증: Bearer Token(파트너 세션), 프리미엄 전용  
...
```json
{
  ...
  "revisitEffect": {
    ...
    "within14Days": 0.0,
    "within30Days": 0.0,
    "nonGameRevisitRate": 0.0,
    "freePlayRevisitRate": 0.0,
    "paidPlayRevisitRate": 0.0
  },
  "isGameActive": true
}
```
...
4: 에러  
　A. 403: 프리미엄 미만  
　B. SM_025: 게임 분포·재방문 조회 실패  


• 참조: 7-2-6-3. 매장 관리 > 3.0. 페이지 개요, 3.1. 상단 고정 영역 ~ 3.7. 하단 CTA·예외·잠금, 3.6. 캠페인 결과 및 다운로드, 3.8. 액션 버튼 및 화면 전환, 7-2-7. 게임 관리  
