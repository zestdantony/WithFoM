...
**2.1.3.7.** 컵홀더 배송·입고 상태 조회 API  

1: 엔드포인트  
　A. `GET /api/v1/partner/campaign/delivery-status`  
　B. 인증: Bearer Token (파트너 세션)  

2: 요청 파라미터  
　A. `campaignId`: 캠페인 ID (필수)  
...
```json
{
  "deliveryStatus": {
    ...
    "stages": {
      "ORDER_RECEIVED": { "completedAt": "2026-02-10T09:00:00+09:00" },
      "MANUFACTURING": { "completedAt": "2026-02-12T14:00:00+09:00" },
      "PREPARING_SHIPMENT": { "completedAt": "2026-02-14T10:00:00+09:00" },
      "IN_TRANSIT": { "startedAt": "2026-02-14T16:00:00+09:00" },
      "RECEIVED": null
    },
    "trackingInfo": {
      "courierName": "CJ대한통운",
      ...
      "trackingEnteredAt": "2026-02-14T16:00:00+09:00"
    },
    "receiptInfo": {
      "isReceived": false,
      "receivedAt": null,
...
4: 응답 필드  
　A. `currentStage`: ORDER_RECEIVED(주문접수), MANUFACTURING(제작중), PREPARING_SHIPMENT(배송준비), IN_TRANSIT(배송중), RECEIVED(입고)  
　B. `trackingInfo`: 협력사가 송장번호 입력 시 활성화 (배송중 단계부터)  
　C. `receiptInfo.receiptMethod`: MANUAL(파트너 수동), AUTO_API(택배사 API), ADMIN(관리자 수동) — 입고 처리 방법 기록  
　D. `trackingInfo.trackingEnteredAt`: 송장번호 입력 시점 (5영업일 미입고 알림 기준일)  


**2.1.3.8.** 배송완료 처리 API  

1: 파트너 수동 배송완료 엔드포인트  
　A. `POST /api/v1/partner/campaign/delivery/confirm-receipt`  
　B. 인증: Bearer Token (파트너 세션)  
...
```js
// 배송완료 처리 (수동/자동 공통 로직)
const processDeliveryComplete = async (campaignId, partnerId, receiptMethod) => {
  // 현재 배송 상태 확인
  const delivery = await getDeliveryStatus(campaignId, partnerId);
  if (delivery.currentStage === 'RECEIVED') {
    return { alreadyProcessed: true }; // 이미 입고 완료 (중복 방지)
  }

  // 입고 처리
  await updateDeliveryStage(campaignId, partnerId, 'RECEIVED', {
    receivedAt: new Date(),
    receiptMethod: receiptMethod // 'MANUAL' | 'AUTO_API' | 'ADMIN'
  });

  // 연쇄 동작: 배포 시작 + 시스템 활성화
  await activateCampaignDistribution(campaignId, partnerId);
  await activateDailyConsumptionReport(campaignId, partnerId);
  await activateInventoryManagement(campaignId, partnerId);
  await activateAlertSystem(campaignId, partnerId);

  // 파트너 알림
  await sendNotification(partnerId, {
    type: 'RECEIPT_COMPLETE',
    message: '입고가 완료되어 캠페인 배포가 시작되었습니다. 오늘부터 일일 소진 보고를 완료해 주세요.'
  });

  return { received: true, receivedAt: new Date(), receiptMethod };
};

// 택배사 API 배송완료 자동 감지 (웹훅/폴링)
const handleCourierDeliveryComplete = async (trackingNumber) => {
  const delivery = await getDeliveryByTracking(trackingNumber);
  if (!delivery || delivery.currentStage === 'RECEIVED') return;

  await processDeliveryComplete(delivery.campaignId, delivery.partnerId, 'AUTO_API');

  await sendNotification(delivery.partnerId, {
    type: 'AUTO_RECEIPT_COMPLETE',
    message: '컵홀더 배송이 완료되어 입고 처리되었습니다.'
  });
};

// 5영업일 미입고 감시 스케줄러
const scheduleUnreceivedAlert = async () => {
  // 매일 실행: 송장번호 입력 후 5영업일 경과, 입고 미처리 건 조회
  const unreceived = await db.query(`
    SELECT * FROM campaign_deliveries
    WHERE current_stage = 'IN_TRANSIT'
      AND tracking_entered_at IS NOT NULL
      AND BUSINESS_DAYS_DIFF(tracking_entered_at, NOW()) >= 5
      AND unreceived_alert_sent = false
  `);

  for (const delivery of unreceived) {
    await sendAdminNotification({
      type: 'UNRECEIVED_URGENT',
      priority: 'URGENT',
      message: `주문 #${delivery.orderId} 송장 입력 후 5영업일 경과, 입고 미처리 상태입니다. 확인이 필요합니다.`,
      campaignId: delivery.campaignId,
      partnerId: delivery.partnerId
    });
    await markUnreceivedAlertSent(delivery.id);
  }
};
```

4: 에러 응답  
　A. 성공: `{ "status": "RECEIVED", "receivedAt": "...", "receiptMethod": "MANUAL" }`  
　B. 실패: `{ "error": "DLV_001", "message": "입고 처리에 실패했습니다." }`  
　C. 이미 처리됨: `{ "error": "DLV_002", "message": "이미 입고 처리된 건입니다." }`  


**2.1.3.9.** 배송·입고 컴포넌트 상태 관리  

1: 상태 정의

```js
const deliveryState = {
  currentStage: 'NONE',  // NONE | ORDER_RECEIVED | MANUFACTURING | PREPARING_SHIPMENT | IN_TRANSIT | RECEIVED
  trackingInfo: null,     // { courierName, trackingNumber, estimatedDeliveryDate, trackingEnteredAt }
  receiptInfo: null,      // { isReceived, receivedAt, receiptMethod }
  isLoading: false,
  error: null
};

const receiptDialogState = {
  isOpen: false,          // 배송완료 확인 팝업
  isProcessing: false
};
```
...
```js
const getDeliveryButtonConfig = (currentStage) => {
  const configs = {
    'ORDER_RECEIVED': { label: '주문접수', color: '#999999', clickable: false },
    'MANUFACTURING': { label: '제작중', color: '#007AFF', clickable: false },
    'PREPARING_SHIPMENT': { label: '배송준비', color: '#007AFF', clickable: false },
    'IN_TRANSIT': { label: '배송중', color: '#FF9500', clickable: false },
    'RECEIVED': { label: '입고', color: '#34C759', clickable: false }
  };
  return configs[currentStage] || configs['ORDER_RECEIVED'];
};

// 배송중 단계에서 배송완료 버튼 추가 표시
const showDeliveryCompleteButton = (currentStage) => {
  return currentStage === 'IN_TRANSIT';
};

// 배송 문의 버튼 노출 (배송중 또는 입고 단계)
const showInquiryButton = (currentStage) => {
  return currentStage === 'IN_TRANSIT' || currentStage === 'RECEIVED';
};
```
...
1: 다이얼로그 접근성  
　A. 확인/포기/참여 중지/배송완료 확인 팝업: `role="dialog"`, `aria-modal="true"`, `aria-labelledby` 지정  
...
4: 배송 상태 버튼 접근성
　A. 읽기 전용 상태 버튼: `role="status"`, `aria-label="현재 배송 상태: {단계명}"` 지정
　B. 배송완료 클릭 가능 버튼: `aria-label="배송 완료 확인"` 지정
　C. 배송 문의 링크: `aria-label="배송 문제 문의하기"` 지정
　D. 배송 상태 변경 시: `aria-live="polite"` 적용 (상태 변경 시 스크린 리더 안내)
...
2: 배송·입고 에러 코드  
　A. `DLV_001`: 입고 처리 실패 → "입고 처리에 실패했습니다. 잠시 후 다시 시도해 주세요." 토스트 표시, 재시도 버튼 제공  
　B. `DLV_002`: 중복 입고 처리 → "이미 입고 처리된 건입니다." 토스트 표시  
　C. `DLV_003`: 배송 상태 조회 실패 → "배송 상태를 불러올 수 없습니다. 잠시 후 다시 시도해 주세요." 토스트 표시  
　D. `DLV_004`: 자동 입고 처리 실패 (시스템 오류) → 자동 재시도 3회 수행, 3회 실패 시 관리자 알림 발송 + 수동 복구 대기, 파트너 화면에는 영향 없음 (백엔드 에러 로깅)  
