...
| unmatched_info | object | 미매칭 상태 정보 (경과 일수, 등급 변경 예정일, 경고 레벨) |
...
| 현재 등급 | "현재 {현재등급} 등급" |
| 등급 변경 등급 | "{다운그레이드등급} 등급으로 조정 예정" |
| 등급 변경 예정일 | "{YYYY-MM-DD} 등급 변경 예정" |
| 남은 기간 | "{N}일 남음" |
...
| CTA 버튼 | 멤버십 상태별 분기: 베이직 "매칭 확률 UP" → 멤버십 구독, 플러스 "프리미엄으로 매칭 확률 UP" → 멤버십 업그레이드, 프리미엄+플래티넘 "매칭 시까지 등급이 유지됩니다" (정보성), 프리미엄+골드이하 "등급 UP으로 매칭 확률 UP" → 등급 현황 |
...
　C. grade.downgrade 이벤트: 등급 마크 변경, 등급 변경 알림 모달 표시  
